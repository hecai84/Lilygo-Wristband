...
class MAX30102(object):
    def __init__(self,
                 ...
                 i2c=I2C(sda=Pin(I2C_DEF_SDA_PIN),
    ...
    def startCheck(self,time,lcd:Display):
        if lcd:
            lcd.PrintText("start check")
        startTime=0
        tempTime=0
        count=0
        lastValue=0
        th_value=10
        th_in=False
        while not startTime or ticks_diff(ticks_ms(),startTime)<time+3000:
            ...
            if self.available():
                ...
                diff=red_reading-lastValue
                lastValue=red_reading
                # 前三秒的数据丢弃
                if ticks_ms()-startTime<3000:
                    self.clear_fifo()
                    continue

                if diff<th_value and diff>-th_value:
                    if not th_in:
                        count+=1
                        th_in=True
                else:
                ...
                if ticks_diff(ticks_ms(),tempTime)>999:
                    tempTime=ticks_ms()
                    if lcd:
                        lcd.PrintText("count:"+str(count))
        
        if lcd:
            lcd.PrintText("heart rate:"+str(count*60000/time))
        return count*60000/time